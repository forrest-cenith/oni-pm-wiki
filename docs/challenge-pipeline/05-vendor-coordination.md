# Vendor Coordination

## Overview
After selection, ONI coordinates with the selected vendor to gather/update the documents needed for contract execution and aligns them to the government SOW/LOEs and ACC-RI requirements. This stage often involves multiple revision cycles.

## Documents Needed to Proceed to Prototype Execution

| Document | Purpose | Owner | Template Link |
|----------|---------|-------|---------------|
| Updated Vendor Proposal | Revised to match government SOW | Vendor | — |
| Pricing Backup / F&R Support | LCAT detail, material costs, cost narrative for Fair & Reasonable determination | Vendor | — |
| W-9 | Tax identification | Vendor | [PDF](https://drive.google.com/file/d/1IDFisM_9c1tUbXDrKV56_QQrySd_fJXh/view?usp=drive_link) |
| Agreement Holder Representations | Vendor certifications (Reps & Certs) | Vendor | [PDF](https://drive.google.com/file/d/1R0-_zE4SbOtbqeTEvwkwz8N3dFJRnTG6/view?usp=drive_link) |

**Coordination Timeline / Status Tracker:** We use the Description field on the Copper CRM card to track this. Example:

```
Current Blocker
3/25 - Forrest reminded CTI for Fair and Reasonable Supporting Docs.

Timeline of Events
3/23 - CTI provided Fair and Reasonable supporting docs.
3/22 - Forrest notified CTI of selection.
```

## Email Templates

<details>
<summary>Selection Notification and Supporting Documentation Request &nbsp; <span class="badge badge-oni">ONI</span> → <span class="badge badge-vendor">Vendor</span></summary>

**Subject:** [Challenge Name] – Selection Notification

**To:** <span class="badge badge-vendor">Vendor</span>

```
[Vendor POC Name],

Congratulations — your proposed solution has been selected
by the Government customer for the [Challenge Name]
opportunity. We appreciate your engagement and the strength
of your submission.

To move forward with the award process, please provide the
following at your earliest convenience:

1. Updated proposal aligned to the attached SOW
   - Ensure alignment with scope, milestones, and deliverables
   - Confirm ROM and payment schedule

2. Pricing backup to support Fair & Reasonable determination
   - LCAT detail with estimated hours and rates
   - Material costs and basis of estimate
   - Brief narrative on key cost drivers and how you arrived
     at your proposed total
   - No formal cost proposal needed — an Excel sheet and/or
     short write-up is sufficient

3. Administrative documents
   - W-9
   - Reps and Certs (template attached)

Please submit ASAP to expedite the review and award process.
Let me know if you have any questions.
```

</details>

<details>
<summary>Fair & Reasonable Pricing Request &nbsp; <span class="badge badge-oni">ONI</span> → <span class="badge badge-vendor">Vendor</span></summary>

**Subject:** [Challenge Name] – Pricing Details Request

**To:** <span class="badge badge-vendor">Vendor</span>

```
[Vendor POC Name],

As part of the award process under OTA, ONI is required to
document a Fair & Reasonable price determination. To support
this, we are requesting a lightweight cost breakdown that
explains how your proposed price was developed.

Please provide the following:

- Labor categories (LCATs) with estimated hours and rates
- Materials or major cost elements with a brief basis of
  estimate (e.g., vendor quotes, prior experience, assumptions)
- Any other key cost drivers (testing, integration, travel, etc.)
- A short summary explaining how you arrived at the total price

An Excel sheet and/or brief write-up is completely fine.
There is no required format or template — high-level but
traceable estimates are appropriate.

Please submit at your earliest convenience to support timely
award processing. Let me know if you have any questions.
```

</details>

## Common Hangups & Resolutions

| Hangup | Resolution |
|--------|------------|
| Vendor thinks their prior updated proposal was sufficient | Give precise change instructions and clarify that the government SOW must be mirrored more closely |
| Government says "there are discrepancies" but doesn't specify | **Major recurring blocker.** Require concrete redlines, examples, or option sets before going back to vendor |
| Cancelled alignment meetings | Replace the missed meeting with a written action list and target return date |
| Admin documents missing | Send standard post-selection checklist immediately after notice of selection |
| Proposal and SOW numbers/ROM mismatch | Always verify pricing alignment when scope documents change |

## Stakeholders & Roles
| Stakeholder | Role |
|-------------|------|
| <span class="badge badge-vendor">Vendor</span> | Updates proposal, submits forms |
| <span class="badge badge-oni">ONI</span> | Coordinates between all parties |
| <span class="badge badge-acc-ri">ACC-RI</span> | Reviews package for contract processing |
| <span class="badge badge-gov">Gov Customer</span> | Validates SOW and scope alignment |

## Typical Timeline
- **Selection notice to updated vendor package:** Days to ~1 week
- **Government/ACC-RI discrepancy identification:** May take additional weeks
- **Realistic PM expectation:** 1-4 weeks for clean package alignment, longer if feedback is vague

## Related Workflows
- [Requesting Updated Proposal](/docs/marketplace-workflows/06-requesting-updated-proposal) — detailed scope alignment guidance for revised proposals
- [Requesting Reps & Certs](/docs/marketplace-workflows/08-requesting-reps-and-certs) — pre-award vendor documentation

## Tips & Lessons Learned
- **Never tell a vendor "please revise" without giving exact instructions**
- Simultaneous coordination works best: ask vendor for readiness while asking government/ACC-RI for precise discrepancy details
- Treat post-selection like a mini requirements reconciliation process
- Send one clean package to ACC-RI rather than scattered iterative fragments
- **While reviewing submissions on the marketplace** to get vendor POC info, go ahead and download their proposal and any reps & certs they uploaded with their submission — drop these into the Copper CRM challenge card so they're easy to find later
